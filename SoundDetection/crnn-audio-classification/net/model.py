    ...
    def predict(self, x, class_name=None):
        with torch.no_grad():
            ...
            for index, name in enumerate(self.classes):
                print(f"Index: {index}, Class Name: {name}")

            # 将 classes 转换为列表
            classes_list = self.classes.tolist() if isinstance(self.classes, np.ndarray) else self.classes

            # 如果提供了 class_name 参数，则返回对应的预测值
            if class_name is not None and class_name in classes_list:
                class_index = classes_list.index(class_name)  # 获取类别名称的索引
                return class_name, out[:, class_index].item()  # 返回指定名称的预测值
            else:
                # 找到概率最大的索引
                max_ind = out.argmax().item()  # 获取概率最大值的索引
                return classes_list[max_ind], out[:, max_ind].item()  # 返回概率最大类别的预测值
