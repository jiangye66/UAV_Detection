...
class AugmentationTransform(object):
    ...
    def _normal_dist(self, x):
        return np.random.normal(0, self.sig, x)

    def _uniform_dist(self, x):
        return np.random.uniform(-self.sig, self.sig, x)

    def _half_dist(self, x):
        return np.clip(
            np.abs(np.random.normal(0, self.sig, x)),
            a_min=0.0,
            a_max=0.8)
